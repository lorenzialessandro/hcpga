...
class GA():
    ...
    def validate_schedule(self, schedule):
        ...
        for machine, tasks in schedule.items():
            for task in tasks:
                job_task_times[task.job_id].append(
                    (task.task_id, task.start_time, task.duration))

        for job_id, task_times in job_task_times.items():
            # Sort by task index
            sorted_tasks = sorted(task_times, key=lambda x: x[0])
            for i in range(len(sorted_tasks) - 1):
                current = sorted_tasks[i]
                next_task = sorted_tasks[i + 1]
    ...
    def repair_chromosome(self, chromosome):
        ...
        # Number of tasks added for each job
        job_added = [0] * self.instance.num_jobs
        for job_id in chromosome:
            # check if job_id is valid
            if job_added[job_id] < self.lengths_jobs[job_id] and job_id < self.instance.num_jobs:
    ...
    def observer(self, population, num_generations, num_evaluations, args):
        ...
        if num_generations % 100 == 0:
            # track best makespan over generations on 100th generation
            self.history_best.append(self.best_makespan)
            # print(
    ...
    def multi_custom_crossover(self, random, candidates, args):
        ...
        for i in range(0, len(candidates) - 1, 2):
            ...
            crossover_points = sorted(
                [random.randint(1, len(parent1) - 2) for _ in range(num_crossover_points)])
            ...
            child1_job_counts = collections.Counter(
                job for job in child1 if job is not None)
            child2_job_counts = collections.Counter(
                job for job in child2 if job is not None)

            # Identify positions that still need to be filled
            child1_empty_positions = [idx for idx,
                                      val in enumerate(child1) if val is None]
            child2_empty_positions = [idx for idx,
                                      val in enumerate(child2) if val is None]
