...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            f'log/solver_log_{datetime.now().strftime("%Y%m%d-%H%M%S")}.log')
    ]
)
...
def run_and_log_experiment(instance, args, run_id, seed, first_run=False):
    ...
    try:
        logger.info(
            f"Running experiment for instance {instance.name} (Run {run_id})")
        ...
        logger.info(
            f"[{instance.name}] [Run {run_id}] Running CP-SAT to find optimal solution...")
        cpsat_make, cpsat_time, cpsat_memory, cpsat_status = run_cpsat(
            instance, seed, args.max_time_budget)
        logger.info(
            f"[{instance.name}] [Run {run_id}] CP-SAT completed: makespan={cpsat_make}, time={cpsat_time:.2f}s, memory={cpsat_memory / 1024 / 1024:.2f}MB")

        # 2. Run HCPGA solver with collector
        logger.info(
            f"[{instance.name}] [Run {run_id}] Running Hybrid solver with collector...")
        hcpga_make, hcpga_time, hcpga_memory = run_hcpga(
            instance, seed, cpsat_time, args.split, args.num_copies, args.num_random, ga_args)
        logger.info(
            f"[{instance.name}] [Run {run_id}] HCPGA completed: makespan={hcpga_make}, time={hcpga_time:.2f}s, memory={hcpga_memory / 1024 / 1024:.2f}MB")

        # 3. Run GA solver
        logger.info(f"[{instance.name}] [Run {run_id}] Running GA solver...")
        ga_make, ga_time, ga_memory = run_ga(
            instance, seed, args.max_time_budget, ga_args)
        logger.info(
            f"[{instance.name}] [Run {run_id}] GA completed: makespan={ga_make}, time={ga_time:.2f}s, memory={ga_memory / 1024 / 1024:.2f}MB")
        ...
        logger.info(
            f"[{instance.name}] [Run {run_id}] Experiment completed and results saved")
        return True

    except Exception as e:
        logger.error(
            f"Error running experiment for {instance.name} (Run {run_id}): {str(e)}")

        # Log error to CSV with thread safety
        with csv_lock:
            with open(args.csv_file, 'a', newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow([instance.name, run_id, seed,
                                "ERROR", str(e), "", "", "", "", "", "", ""])
...
def run_hcpga(instance, seed, time_budget, split, num_copies, num_random, ga_args):
    ...
    try:
        hybrid_solver = HCPGA(instance, seed=seed, time_budget=time_budget, split=split,
                              num_copies=num_copies, num_random=num_random, ga_args=ga_args)
        ...
        logger.error(
            f"Error in hybrid collector for {instance.name}: {str(e)}")
...
def run_ga(instance, seed, time_budget, ga_args):
    ...
    try:
        ...
        schedule, makespan, tot_time, memory_used = ga_solver.solve(
            args=ga_args)
...
def process_instance(file_path, args, worker_id):
    ...
    for run_id in range(num_runs):
        ...
        logger.info(
            f"Worker {worker_id}: Starting run {run_id+1}/{num_runs} for instance {instance_name} with seed {run_seed}")
        success = run_and_log_experiment(
            instance, args, run_id, run_seed, first_run=(run_id == 0 and worker_id == 0))
        if success:
            success_count += 1
...
def args_parser():
    ...
    parser = argparse.ArgumentParser(
        description='Solve and log Job Shop Problem Solver using HCPGA')
    parser.add_argument('--folder', type=str, help='Path to the folder containing the instances',
                        default='instances/ClassicBenchmark')
    parser.add_argument('--csv_file', type=str,
                        default=f'csv/results_{datetime.now().strftime("%Y%m%d-%H%M%S")}.csv', help='CSV file to log the results')
    parser.add_argument('--max_time_budget', type=int,
                        help='Max time budget for each solver in seconds', default=10800)
    parser.add_argument('--seed', type=int,
                        help='Base random seed', default=10)
    parser.add_argument('--num_runs', type=int,
                        help='Number of runs per instance with different seeds', default=1)
    parser.add_argument('--workers', type=int,
                        help='Number of parallel workers (default: 1)', default=1)
    parser.add_argument('--split', type=float,
                        help='Split of time budget between CP-SAT and GA solvers', default=0.3)
    parser.add_argument('--num_copies', type=int,
                        help='Number of copies of the base chromosome to add to the population', default=1)
    parser.add_argument('--num_random', type=int,
                        help='Number of random chromosomes to add to the population', default=30)
    parser.add_argument('--pop_size', type=int,
                        help='Population size for GA solver', default=100)
    parser.add_argument('--max_generations', type=int,
                        help='Max of generations for GA solver', default=1000)
    parser.add_argument('--mutation_rate', type=float,
                        help='Mutation rate for GA solver', default=0.7)
    parser.add_argument('--crossover_rate', type=float,
                        help='Crossover rate for GA solver', default=0.6)
    parser.add_argument('--num_selected', type=int,
                        help='Number of selected chromosomes for GA solver', default=70)
    parser.add_argument('--num_elites', type=int,
                        help='Number of elite chromosomes for GA solver', default=10)
    parser.add_argument('--tournament_size', type=int,
                        help='Tournament size for GA solver', default=5)
    parser.add_argument('--patience', type=int,
                        help='Patience for GA solver', default=3)
...
def main():
    ...
    if args.folder is not None:
        ...
        logger.info(
            f"Starting parallel processing of {len(instance_files)} instances with {max_workers or 'default'} workers")
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_instance):
                ...
                try:
                    ...
                    completed += 1
                    logger.info(
                        f"Progress: {completed}/{len(instance_files)} instances completed. {instance_name}: {success_count}/{total_runs} runs succeeded")
                except Exception as e:
                    logger.error(
                        f"Instance {instance_name} generated an exception: {str(e)}")
